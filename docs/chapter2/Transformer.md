...
对于输入的一组序列，对于每个Query向量 $Q_i$，我们需要与所有的 $Key_j$ 向量进行点乘，共有 n 个Key向量。每次点乘操作的计算复杂度是 $O(d)$,因此，对于每个Query向量，计算所有点乘操作的总复杂度是 $O(n*d)$ ，由于序列中有 n 个Query向量，每个都需要进行这样的操作，所以总计算复杂度是 
$O(n^2 * d)$  
